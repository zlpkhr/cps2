...
ROWS=$(grep -c . "$startfile")
HEADLINE=$(head -n 1 "$startfile")
COLS="${#HEADLINE}"
echo "$ROWS $COLS"
                        ...
                        # $2) cell number
{
  ...
  do
    let "t_top = ($top + $i + $ROWS * $COLS) % ($ROWS * $COLS)"
    let "t_cen = ($center + $i + $ROWS * $COLS) % ($ROWS * $COLS)"
    let "t_bot = ($bottom + $i + $ROWS * $COLS) % ($ROWS * $COLS)"
